...
<template>
  ...
  <div v-else-if="role === 'assistant'" class="space-y-1">
    ...
    <div
      ...
      :class="
        decisionFor(content, tc.id) === false
          ...
          : decisionFor(content, tc.id) === true
    ...
    >
      ...
      <div
        v-if="requiresApproval(content) && decisionFor(content, tc.id) === undefined"
    ...
    <div
      v-if="requiresApproval(content) && toolCalls(content).length > 1"
    ...
    >
      <button
        ...
        :disabled="assistant.sending"
        @click="decideAll(toolCalls(content), true)"
      >
        Approve all
      </button>
      <button
        ...
        @click="decideAll(toolCalls(content), true, true)"
      >
        Always allow all
      </button>
      <button
        ...
        :disabled="assistant.sending"
        @click="decideAll(toolCalls(content), false)"
      >
        Reject all
      </button>
      <button
        ...
        @click="decideAll(toolCalls(content), false, true)"
      >
        Always reject all
      </button>
    </div>
