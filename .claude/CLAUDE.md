...
- **Export:** `mdcast` (`src/export/`) renders pages to PDF/PDF-slides in-process via `typst`/`typst-as-lib`, and to DOCX/ODT/PPTX/reveal.js-slides via a `pandoc` subprocess, pulling templates/brand config from `design/` and images from `file_blobs` through `DbAssetProvider`. `markdown::render_for_export` + `export::asset_provider` (#66) bridge every markdown directive (`<page>`/`<file>`/`<image>`/`<gallery>`/`<fen>`/`<pgn>`/`<mermaid>`/`<json>`) to plain markdown — real image refs/pipe tables/spliced page markdown, with synthesized fen/pgn/mermaid diagrams (`chess-diagram`/`mermaid-svg`) layered in as assets — since typst can't consume raw HTML; see [`docs/architecture.md`](../docs/architecture.md#export-mdcast) (#64, #65, #66; export routes land in follow-up issues)
