...
- **Export:** `mdcast` (`src/export/`) renders pages to PDF/PDF-slides in-process via `typst`/`typst-as-lib`, and to DOCX/ODT/PPTX/reveal.js-slides via a `pandoc` subprocess, pulling templates/brand config from `design/mdcast/` and images from `file_blobs` through `DbAssetProvider`. `markdown::render_for_export` + `export::asset_provider` (#66) bridge every markdown directive (`<page>`/`<file>`/`<image>`/`<gallery>`/`<fen>`/`<pgn>`/`<mermaid>`/`<json>`) to plain markdown — real image refs/pipe tables/spliced page markdown, with synthesized fen/pgn/mermaid diagrams (`chess-diagram`/`mermaid-svg`) layered in as assets — since typst can't consume raw HTML. Its `mdcast::BrandSpec` (palette/fonts) is sourced from `design/mdcast/brand.toml`, mapped from `design/assets/css/style.css`'s `:root` tokens, with brand-aware typst layouts (`design/mdcast/typst/layouts/pdf/*.typ`) and a reveal.js theme escape hatch (`design/mdcast/revealjs/brand.css`) alongside it — all `DESIGN_DIR`-overridable like every other design asset; see [`docs/architecture.md`](../docs/architecture.md#export-mdcast) (#64, #65, #66, #67, #68)
...
| `MDCAST_PANDOC_PATH` | `pandoc` | Override the `pandoc` binary path/name mdcast's DOCX/ODT/PPTX/reveal.js-slides export backends spawn (#64) |
