...
- **Routing:** public SSR catch-all + `/assets` (`src/routes/public/`), session-cookie JSON API under `/api/*` (`src/routes/api/`, guarded by `require_login_api`), OAuth2 (`src/routes/oauth.rs`), and the MCP endpoint `POST /mcp` (`src/routes/mcp/`).
