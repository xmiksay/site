...
- Unit tests live in `src/` (`#[cfg(test)]`, e.g. `templates.rs`, `design.rs`, `markdown/tests.rs`); there are no integration tests yet — `make test-integration` no-ops until `tests/` exists. Add tests with new logic.
